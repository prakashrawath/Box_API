...
def upload_file(file_name: str, shared_link: bool, sub_folder: str, folder_det: bool, zip: str):
    ...
    # Get folder details
    if folder_det:
        response = requests.get('https://api.box.com/2.0/folders/134405394827', headers=headers)
        print(response.json())
    # shared link
    if shared_link:
        response = requests.put('https://api.box.com/2.0/folders/134405394827?fields=shared_link', headers=headers)
        if response.ok:
    ...
    if file_name:
        # Upload file
        data = {'attributes': '{"name":"file_name", "parent":{"id":"134405394827"}}'}
